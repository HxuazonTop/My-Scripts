...
wait(1)
-- init
local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/leg1337/legendhub/main/legendhub%20additional.lua"))()
local venyx = library.new("legendhub v1.0", 5013109572)
...
-- first page
local page = venyx:addPage("main tab", 5012544693)
local section1 = page:addSection("visualizer")
local section2 = page:addSection("visual settings")
section1:addTextbox("id", "0", function(value, enterPressed)
...
section1:addTextbox("visualizer target", "name", function(value, enterPressed)
...
section1:addButton("play/sync/massplay", function()
    if vis == false then
		for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
...
section2:addTextbox("distance", "0", function(value, enterPressed)
...
section2:addTextbox("height", "0", function(value, enterPressed)
...
section2:addTextbox("speed", "0", function(value, enterPressed)
    
if enterPressed then
sin.Value = value
venyx:Notify("speed", "speed set to "..value)
end
end)

section2:addTextbox("up/down/beat", "0", function(value, enterPressed)
...
section4:addButton("spin", function()
    local waitTime = 1/1
	vis = true
    ...
    venyx:Notify("visualizer", "started spin visualizer")
end)
...
section4:addButton("circle 2", function()
    local waitTime = 1/1
	vis = true
	Index = 360 / #game.Players.LocalPlayer.Backpack:GetChildren()
	game.Players.LocalPlayer.Character.Animate.toolnone.ToolNoneAnim.AnimationId = "0"
	for i, v in next, getSelection() do
		if v:IsA("Tool") then
			...
			bp.MaxForce = Vector3.new(math.huge,math.huge,math.huge)
			bp.P = 10
			bp.Position = game.Players.LocalPlayer.Character.Torso.Rotation
			local bg = Instance.new("BodyGyro", v.Handle)
			bg.MaxTorque = Vector3.new(math.huge,math.huge,math.huge)
			bg.P = 1.0
			local val = Instance.new("NumberValue", v)
			...
			at1.Name = "at1"
			at2.Name = "at2"
			at1.Rotation = Vector3.new(-1.4, 0, 0)
			at2.Position = Vector3.new(1.4, 0, 0)
			yv.Value = k
			k = yv.Value + k
			val.Value = x
			...
			secrot = 0.1
			visual = -8
			local angle = Vector3.new(-5,math.rad(22),0)
			local Connection
			Connection = game:GetService("RunService").Heartbeat:Connect(function()
				bp.D = d
				rot = rot+sin.Value*0.053
				local tools = getSelection()
				v.Handle.Velocity = Vector3.new(2e2,0,0)
...
section4:addButton("wave", function()
    local waitTime = 1/1
	vis = true
	Index = 360 / #game.Players.LocalPlayer.Backpack:GetChildren()
	game.Players.LocalPlayer.Character.Animate.toolnone.ToolNoneAnim.AnimationId = "0"
    ...
    venyx:Notify("visualizer", "started wave visualizer")
end)
section4:addButton("z position", function()
    local waitTime = 1/1
	vis = true
	Index = 360 / #game.Players.LocalPlayer.Backpack:GetChildren()
	game.Players.LocalPlayer.Character.Animate.toolnone.ToolNoneAnim.AnimationId = "0"
	for i, v in next, getSelection() do
		if v:IsA("Tool") then
			...
			Connection = game:GetService("RunService").Heartbeat:Connect(function()
				...
				local tools = getSelection()
				v.Handle.Velocity = Vector3.new(2e2,0,0)
				v.Handle.Rotation = Vector3.new(22+rot,0,0+rot)
				v.Handle.Position = game.Players[player].Character.Head.Position
				v.Handle.Position = game.Players[player].Character.Head.Position + Vector3.new(math.sin(math.rad(val.Value+song.PlaybackLoudness/50) + time() * sin.Value) * off.Value, ym.Value + math.abs(math.sin(val.Value/3 + tick()/3 * math.pi)) * 400 * 0.005 * pbl.Value, math.cos(math.rad(val.Value+song.PlaybackLoudness/250) + time() * sin.Value) * off.Value)
				bp.Position = game.Players[player].Character.Head.Position + Vector3.new(math.sin(math.rad(val.Value+song.PlaybackLoudness/250) + time() * sin.Value) * off.Value, ym.Value + math.abs(math.sin(val.Value/3 + tick()/3 * math.pi)) * 400 * 0.005 * pbl.Value, math.cos(math.rad(val.Value+song.PlaybackLoudness/250) + time() * sin.Value) * off.Value)
			    if v.Handle.Parent == game.Workspace then
		    else
			    v.Handle.CFrame = CFrame.new(v.Handle.Position, Vector3.new(game.Players[player].Character.Head.Position.Y, game.Players[player].Character.Head.Position.Z - 4, game.Players[player].Character.Head.Position.Y)) * CFrame.Angles(0,0,0) * CFrame.new(0,0,0+v.Handle.Sound.PlaybackLoudness/100)
				if v.Parent == game.Players.LocalPlayer.Backpack then
				    vis = false
				    Connection:Disconnect()
				end
    ...
    venyx:Notify("visualizer", "started z position visualizer")
end)
section4:addButton("circle 3", function()
    local waitTime = 1/1
	vis = true
	Index = 360 / #game.Players.LocalPlayer.Backpack:GetChildren()
	game.Players.LocalPlayer.Character.Animate.toolnone.ToolNoneAnim.AnimationId = "0"
	for i, v in next, getSelection() do
		if v:IsA("Tool") then
			...
			Connection = game:GetService("RunService").Heartbeat:Connect(function()
			    ...
			    v.Handle.CFrame = CFrame.new(v.Handle.Position, Vector3.new(game.Players[player].Character.Head.Position.X, game.Players[player].Character.Head.Position.Y - 0, game.Players[player].Character.Head.Position.Z)) * CFrame.Angles(20+v.Handle.Sound.PlaybackLoudness/600,0,0) * CFrame.new(0,0,0+v.Handle.Sound.PlaybackLoudness/300)
				if v.Parent == game.Players.LocalPlayer.Backpack then
				    vis = false
				    Connection:Disconnect()
				end
    ...
    venyx:Notify("visualizer", "started circle 3 visualizer")
end)
section4:addButton("circle line", function()
    local waitTime = 1/1
	vis = true
	Index = 360 / #game.Players.LocalPlayer.Backpack:GetChildren()
	game.Players.LocalPlayer.Character.Animate.toolnone.ToolNoneAnim.AnimationId = "0"
	for i, v in next, getSelection() do
		if v:IsA("Tool") then
			...
			Connection = game:GetService("RunService").Heartbeat:Connect(function()
				...
				v.Handle.Velocity = Vector3.new(2e2,0,0)
				v.Handle.Rotation = Vector3.new(0+rot,0,0+rot)
				v.Handle.Position = game.Players[player].Character.Head.Position + Vector3.new(math.sin(math.rad(val.Value+v.Handle.Sound.PlaybackLoudness/1) + time() * sin.Value) * off.Value, ym.Value + math.abs(math.sin(yv.Value + time() * math.pi)) * v.Handle.Sound.PlaybackLoudness * 0.005 * pbl.Value, math.cos(math.rad(val.Value+v.Handle.Sound.PlaybackLoudness/250) + time() * sin.Value) * off.Value)
			    if v.Handle.Parent == game.Workspace then
		    else
			    v.Handle.CFrame = CFrame.new(v.Handle.Position, Vector3.new(game.Players[player].Character.Head.Position.X, game.Players[player].Character.Head.Position.Y - 4, game.Players[player].Character.Head.Position.Z)) * CFrame.Angles(0+v.Handle.Sound.PlaybackLoudness/600,0,0) * CFrame.new(0,0,0+v.Handle.Sound.PlaybackLoudness/300)
				if v.Parent == game.Players.LocalPlayer.Backpack then
    ...
    venyx:Notify("visualizer", "started circle line visualizer")
end)
section4:addButton("simple line", function()
    local waitTime = 1/1
	vis = true
	Index = 360 / #game.Players.LocalPlayer.Backpack:GetChildren()
	game.Players.LocalPlayer.Character.Animate.toolnone.ToolNoneAnim.AnimationId = "0"
	for i, v in next, getSelection() do
		if v:IsA("Tool") then
			...
			Connection = game:GetService("RunService").Heartbeat:Connect(function()
				...
				v.Handle.CFrame = game.Players[player].Character.Torso.CFrame * CFrame.new(i * 1.6 - (#tools * 1.6 / 2), math.abs(math.sin(yv.Value + time() * math.pi)) * pbl.Value * v.Handle.Sound.PlaybackLoudness * 0.005,0) * CFrame.new(10,0+ym.Value,0) * CFrame.Angles(0,math.rad(2.3),math.rad(2.3))
				if v.Parent == game.Players.LocalPlayer.Backpack then
				    vis = false
				    Connection:Disconnect()
				end
    ...
    venyx:Notify("visualizer", "started simple line visualizer")
end)
...
creds:addTextbox("legendary#0001 thx for using")
-- load
